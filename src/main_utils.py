...
# set .tsv dir
def get_tsv_dirs(dataset):
    log_directory = os.path.join("data", dataset)
    tsv_name = dataset + "_sequences.tsv"
    return os.path.join(log_directory,tsv_name)
...
def setup_logging_directories(dataset, run_name, embedding_string):
    ...
    if not os.path.exists(log_path):
        with open(log_path, 'w', newline='') as log_file:
            ...
            log_writer.writerow(['Dataset', 'Embedding', 'Hidden Dimension', 'Seed', 'Best Val Metric', 'Test Metric', 'Parameter Count', 'Max Memory', 'Run Time', 'Time'])
...
def setup_ppi_logs(dataset, run_name):
    log_directory = os.path.join("results", "ppi_results_exp")
    os.makedirs(log_directory, exist_ok=True)
...
def setup_cath_logs():
    log_directory = os.path.join("results", "cath_results_exp")
    os.makedirs(log_directory, exist_ok=True)
...
def load_embeddings(embedding_string, task):#把论文中提到的那些 tpLM/基础 pLM 的预生成 embedding（如 OntoProtein、ProteinCLIP、ESM3 等）按你在命令行或脚本中设定的简写字符（B、C、D…）一个个加载进来
    ...
    return dictionaries #把所有成功加载到的 embedding 字典列表返回
